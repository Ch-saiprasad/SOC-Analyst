# 🛡️ SOC Analyst Journey

![Status](https://img.shields.io/badge/Status-Learning-blue)
![Platform](https://img.shields.io/badge/Platform-TryHackMe-red)
![Focus](https://img.shields.io/badge/Focus-Blue%20Team-darkblue)

## 👋 About Me

Hi! I'm an aspiring **SOC (Security Operations Center) Analyst** based in Hyderabad, India.  
I'm self-learning cybersecurity with a focus on **Blue Team / Defensive Security**.

- 🎯 Goal: Land a SOC L1 Analyst role
- 💻 Background: Familiar with Linux & Windows
- 📚 Currently: Studying SOC Level 1 on TryHackMe
- 📍 Location: Hyderabad, India

---

## 🗺️ Learning Roadmap

| Phase | Topic | Status |
|-------|-------|--------|
| ✅ Phase 1 | Linux & Windows Fundamentals | Done |
| ✅ Phase 2 | Networking Basics (TCP/IP, DNS, HTTP) | Done |
| 🔄️ Phase 3 | SIEM & Log Analysis (Splunk) | In Progress |
| ⏳ Phase 4 | Endpoint Security & Monitoring | Upcoming |
| ⏳ Phase 5 | Phishing Analysis & Threat Intelligence | Upcoming |
| ⏳ Phase 6 | Hands-on SOC Simulations (LetsDefend) | Upcoming |

---

## 🧰 Tools I'm Learning

| Tool | Purpose |
|------|---------|
| Wireshark | Network packet analysis |
| Splunk | SIEM & log analysis |
| VirusTotal | Malware & URL analysis |
| Sysmon | Windows event logging |
| Nmap | Network scanning basics |

---

## 📂 Repository Structure

```
SOC-Analyst-Journey/
│
├── README.md                  ← You are here
├── TryHackMe-Notes/           ← Notes from each TryHackMe room
├── Writeups/                  ← Room walkthroughs & investigations
├── Tools/                     ← Notes on tools like Wireshark, Splunk
└── Resources.md               ← Useful links, cheatsheets & references
```

---

## 🏅 TryHackMe Progress

- 🔗 TryHackMe Profile: [tryhackme.com/p/saiprasadchalpuri0](https://tryhackme.com/p/saiprasadchalpuri0)
- Rooms Completed
  - [ ] Pre-Security Path
  - [ ] Network Fundamentals
  - [ ] Linux Fundamentals 1, 2, 3
  - [ ] Cyber Defence Frameworks
  - [ ] Intro to SIEM
  - [ ] Splunk Basics
  - [ ] Wireshark Basics
  - [ ] Phishing Analysis

---

## 📬 Connect With Me

- 💼 LinkedIn: [www.linkedin.com/in/chalpuri-sai-prasad1808]
- 🔐 TryHackMe: [https://tryhackme.com/p/saiprasadchalpuri0]

---

> *"Security is not a product, but a process."* — Bruce Schneier
